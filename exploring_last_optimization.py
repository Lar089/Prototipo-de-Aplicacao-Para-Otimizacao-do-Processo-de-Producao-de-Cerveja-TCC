...
import pickle
with open('last_optimization.pickle', 'rb') as handle:
    Cervejaria = pickle.load(handle)
...
def plot(scenario,img_filename=None,resource_height=1.0,show_task_labels=True,
    ...
    df = pd.DataFrame(solution, columns=['Tarefa', 'Recurso', 'Inicio','Fim'])	
	return plt, solution, df

#plt, solution = plot(Cervejaria, fig_size=(30, 10))
#import pandas as pd
